...
unsigned char consoleGetSysCmdNameParm(unsigned char *pScr, unsigned char nlen,unsigned char parmLenMax,unsigned char parmMax)
{
   ...
   {  
        ...
        {
          stConsoleDev.funcParmTbl[stConsoleDev.funcParmn-1][i++]='\0';//每个参数加上结束符
          i=0;//清除参数的长度	  
		...
		{
		   //对参数进行过滤
		   //if('a'<=pBuf[len]<='z'||'A'<=pBuf[len]<='Z'||'0'<=pBuf[len]<='9')
	       stConsoleDev.funcParmTbl[stConsoleDev.funcParmn-1][i++]=pBuf[len];//保存参数
		   if(i>parmLenMax) 
		   	return 1;//大于最大长度
		}
   }
