...
/*罗校敬*/
//extern u8 USART_printf;/*选择哪个串口作为printf输出*/
void all_uart_init(void);
u16 GetCrc(const u8 *pSendBuf, u8 nEnd);
//void sz_memset(char *buf,char value,int len);


//参考《x2_Transponder》
extern int usart1_read(char *src,int data_len);
extern int usart1_write(char *src,int data_len);
extern int handle_data_uart(u8 uart,unsigned char *pData,int plen);
